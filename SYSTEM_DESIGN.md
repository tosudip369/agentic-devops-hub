...
3.  **Contextual Attention:** The next time `observer.sh` detects a failure, it queries the memory bank. If the error signature matches a past failure, the Hub injects the historical context into the Remediator agent's prompt: *"ATTENTION: You have seen this before. In PR #42, the solution was X. Do not attempt Y."*
...
*   **Antigravity CLI (`agy`):** The `remediate.sh` script is natively powered by `agy`. You can manually dispatch the hub's learning capabilities from anywhere using `agy run agentic-hub-fix --context=$(pwd)`.
