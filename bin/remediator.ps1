...
if ($failed) {
    ...
    Write-Host "☠️ INITIATING PATH OF EXILE MINION SWARM (V10 EXECUTION LOOP)..." -ForegroundColor Magenta

    # === 3. SUMMON GOLEM (TDD ENGINEER) ===
    Write-Host "   -> 🪨 Summoning Golem (Test Engineer)..." -ForegroundColor Blue
    $testPrompt = "[ROLE: GOLEM] You are a TDD Golem. Write a PowerShell test script to verify the fix for $errorMsg in $FilePath. OUTPUT RAW CODE ONLY."
    ...
    while ($loopCount -lt $maxRetries -and -not $testPassed) {
        ...
        if ($loopCount -eq 1) {
            $devPrompt = "[ROLE: SKELETON] The file $FilePath crashed with: $errorMsg. Write the exact replacement code to fix this. OUTPUT ONLY RAW CODE."
        } else {
            $devPrompt = "[ROLE: SKELETON] The file $FilePath crashed. Your previous fix failed the Golem's test with error: $testError. Rewrite the exact replacement code to pass the test. OUTPUT ONLY RAW CODE."
        }
        
        $devFix = agy.exe --print $devPrompt --dangerously-skip-permissions
        $devFix = $devFix -replace '`\w*\r?\n', '' -replace '`', ''
        ...
        $testOutput = powershell -NoProfile -NonInteractive -Command "& '$testPath'" 2>&1
    ...
    $reviewPrompt = "[ROLE: ZOMBIE] You are a Gatekeeper Zombie. Review this test-verified fix for $FilePath:
...
Analyze for O(1) performance and security. If flawless, output 'APPROVED'. If flawed, output CORRECTED RAW CODE."
    ...
    if ($finalFix.Trim() -match "APPROVED") {
        ...
        $finalCode = $finalFix -replace '`\w*\r?\n', '' -replace '`', ''
    }
