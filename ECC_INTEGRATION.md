...
| **Autonomy Level** | Requires human invocation (`/ecc:plan`, etc.). | **observer.sh & remediate.sh**: Fully autonomous. The system watches itself and spawns agents when it breaks. |
...
1. **The Observer Detects a Failure**: Our `bin/observer.sh` spots a broken build or syntax error.
2. **The Remediator Spawns**: `bin/remediate.sh` checks out an isolated branch (via `treehouse` concepts).
